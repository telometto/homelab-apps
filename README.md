...
- `storage/` — Single-node local storage class for KubeVirt pilot disks
...
The first applied VM is `vms/actual`, which is intentionally halted until the
host-side local PV directory exists and the operator stack is healthy.
